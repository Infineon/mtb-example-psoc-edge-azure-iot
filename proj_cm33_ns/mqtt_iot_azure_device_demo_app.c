...
* Macros
*******************************************************************************/
#define MAX_TELEMETRY_MESSAGE_COUNT                 (5U)
#define TELEMETRY_SEND_INTERVAL_SEC                 (1U * 1000U)
#define MAX_MESSAGE_COUNT                           (100U)

/* Wait duration between messages, should be in multiples of 5 */
#define MESSAGE_WAIT_LOOP_DURATION_SEC              (5U)

#define MESSAGE_WAIT_DELAY_MSEC                     (1000U * 5U)

#define SAS_KEY_DURATION_MINUTES                    (240U)

#define MQTT_CLIENT_ID_BUFFER_SIZE                  (128U)

#define TELEMETRY_TOPIC_BUFFER_SIZE                 (128U)

/* Defines for methods app */
#define METHODS_RESPONSE_TOPIC_BUFFER_SIZE          (128U)

#define GET_QUEUE_TIMEOUT_MSEC                      (500U)

#define PUT_QUEUE_TIMEOUT_MSEC                      (500U)

#define METHOD_WAIT_LOOP_DURATION_MSEC              (120U * 1000U)

/* Defines for twin app */
#define MAX_TWIN_MESSAGE_COUNT                      (5U)

#define MESSAGE_SEND_RECEIVE_INTERVAL_MSEC          (10U * 1000U)

#define GET_SEMAPHORE_TIMEOUT_MSEC                  (500U)

#define DEVICE_TWIN_WAIT_LOOP_DURATION_MSEC         (120U * 1000U)

#define DEVICE_TWIN_TIMEOUT_MSEC                    (500U)

#define TWIN_MESSAGE_WAIT_DELAY_MSEC                (1000U)

#define DEVICE_DEMO_APP_TIMEOUT_MSEC                (5U)

#define HUB_DIRECT_METHOD_EVENT_QUEUE_LENGTH        (10U)

#define C2D_FEATURE                                 (0U)
#define METHODS_FEATURE                             (1U)
#define DEVICE_TWIN_FEATURE_PATCH                   (2U)
#define DEVICE_TWIN_FEATURE_RESPONSE                (3U)
...
static cy_rslt_t send_telemetry_messages_to_iot_hub(void)
{
    ...
    for( uint8_t message_count = 0; message_count < MAX_MESSAGE_COUNT; message_count++ )
    {
        ...
        vTaskDelay(pdMS_TO_TICKS(TELEMETRY_SEND_INTERVAL_SEC));
    }
    return CY_RSLT_SUCCESS;
}
...
static cy_rslt_t subscribe_azure_hub_features_topics(void)
{
    ...
    sub_msg[C2D_FEATURE].qos = (cy_mqtt_qos_t)CY_MQTT_QOS1;
    sub_msg[C2D_FEATURE].topic = AZ_IOT_HUB_CLIENT_C2D_SUBSCRIBE_TOPIC;
    sub_msg[C2D_FEATURE].topic_len = ( ( uint16_t ) ( sizeof( AZ_IOT_HUB_CLIENT_C2D_SUBSCRIBE_TOPIC ) - 1 ) );

    /* Subscription topic and parameters for Methods feature. */
    sub_msg[METHODS_FEATURE].qos = (cy_mqtt_qos_t)CY_MQTT_QOS1;
    sub_msg[METHODS_FEATURE].topic = AZ_IOT_HUB_CLIENT_METHODS_SUBSCRIBE_TOPIC;
    sub_msg[METHODS_FEATURE].topic_len = ( ( uint16_t ) ( sizeof( AZ_IOT_HUB_CLIENT_METHODS_SUBSCRIBE_TOPIC ) - 1 ) );

    /* Subscription topic and parameters for Device Twin Patch,
     * for retrieving desired twin property. */
    sub_msg[DEVICE_TWIN_FEATURE_PATCH].qos = (cy_mqtt_qos_t)CY_MQTT_QOS1;
    sub_msg[DEVICE_TWIN_FEATURE_PATCH].topic = AZ_IOT_HUB_CLIENT_TWIN_PATCH_SUBSCRIBE_TOPIC;
    sub_msg[DEVICE_TWIN_FEATURE_PATCH].topic_len = sizeof( AZ_IOT_HUB_CLIENT_TWIN_PATCH_SUBSCRIBE_TOPIC ) -1 ;

    /* Subscription topic and parameters for Device Twin Response for device twin response. */
    sub_msg[DEVICE_TWIN_FEATURE_RESPONSE].qos = (cy_mqtt_qos_t)CY_MQTT_QOS1;
    sub_msg[DEVICE_TWIN_FEATURE_RESPONSE].topic = AZ_IOT_HUB_CLIENT_TWIN_RESPONSE_SUBSCRIBE_TOPIC;
    sub_msg[DEVICE_TWIN_FEATURE_RESPONSE].topic_len = sizeof( AZ_IOT_HUB_CLIENT_TWIN_RESPONSE_SUBSCRIBE_TOPIC ) -1 ;
...
static cy_rslt_t create_and_configure_mqtt_client(void)
{
    ...
    result = cy_mqtt_create( buffer, NETWORK_BUFFER_SIZE,
            security, &broker_info,"TEST",
            &mqtthandle );
    result = cy_mqtt_register_event_callback(mqtthandle, mqtt_event_cb, NULL);
    if( TEST_PASS == result )
    {
